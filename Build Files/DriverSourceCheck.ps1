...
$Drivers = get-cmdriver -DriverPackageid $pkgid -fast
$pkgname | out-file C:\temp\drivers.txt
$Drivers | out-file C:\temp\drivers.txt -Append

$missing = [system.collections.arraylist] @()


ForEach($Driver in $Drivers){
    if(!(test-path -path ("filesystem::"+$Driver.ContentSourcePath))){$missing.add($Driver) | out-null}
}

$missing | select LocalizedDisplayName, ContentSourcePath
if($missing.count -gt 0){
 # Create a stackpanel container
        ...
        $TextBlock.Text = "The following Driver(s) are missing from their source path."
        $TextBlock.Margin = 10
...
$DataGrid.GridLinesVisibility = "None"
        $TextBlock, $DataGrid | ForEach-Object {
        ...
        # Create a textblock
        $TextBlock = New-Object System.Windows.Controls.TextBlock
        $TextBlock.Text = "All drivers in package $($pkgid) were found at their source paths"
        $TextBlock.Margin = 10
